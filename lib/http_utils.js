 ...
 * @param {null|String} queryString to append to the url
 * @return {Object} HTTP client request object filled with common options
 */
function getCommonStoreRequestOptions(
    httpAgent, hostname, port, key, queryString = '') {
    const hasQS = queryString !== '' ? '?' : '';
    return {
        hostname,
        port,
        path: `${protocol.specs.STORAGE_BASE_URL}/${key}${hasQS}${queryString}`,
        agent: httpAgent,
        protocol: 'http:',
