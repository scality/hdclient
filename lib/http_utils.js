 ...
 * @param {boolean|undefined|null} immutable check (PUT only)
 * @return {Object} HTTP client request object filled with common options
 */
function getCommonStoreRequestOptions(
    httpAgent, hostname, port, key, immutable = false) {
    const queryString = immutable ? '?immutable=true' : '';
    return {
        hostname,
        port,
        path: `${protocol.specs.STORAGE_BASE_URL}/${key}${queryString}`,
        agent: httpAgent,
        protocol: 'http:',
