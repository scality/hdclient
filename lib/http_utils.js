 ...
 *         -timeout {boolean}
 */
function makeOperationContext(fragments) {
    const nParts = fragments.nDataParts + fragments.nCodingParts;
    const opContext = {
        fragments,
        status: [...Array(fragments.nChunks).keys()].map(
            () => ({
                nOk: 0,
                nError: 0,
                nTimeout: 0,
...
function _updateOperationContext(reqContext,
                                 { response = null,
                                   error = null,
                                   timeout = false }) {
    const chunk = reqContext.opContext.status[reqContext.chunkId];
    assert.strictEqual(chunk.statuses[reqContext.fragmentId], undefined);
 ...
 * @returns {Any} Anything callback returned
 */
function _handleHttpSuccess(reqContext, callback, response) {
    _updateOperationContext(reqContext, { response });
    const ret = callback(reqContext.opContext);
...
function _handleHttpError(reqContext, callback, errInfos) {
    ...
    _updateOperationContext(reqContext, { error });
    return callback(reqContext.opContext);
}
 ...
 * @returns {http.ClientRequest} created request
 */
...
function newRequest(options, logger, reqContext, timeoutMs, callback) {
    ...
    request.setTimeout(timeoutMs, () => {
        ...
        error.errInfos = enhanceLogs;
        _updateOperationContext(reqContext, { error, timeout: true });
        const ret = callback(reqContext.opContext);
        request.abort();
        return ret;
    });
