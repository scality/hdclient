...
function validateCodeSection(codes, nLocations) {
    ...
    for (let i = 0; i < codes.length; i++) {
        ...
        if (nLocations < code.dataParts + code.codingParts) {
            return {
                configIsValid: false,
                configError: new InvalidConfigError(
                    'totalParts',
                    code.dataParts + code.codingParts,
                    `Expected less parts than data locations (code ${i})`),
            };
        }
        ...
        code.regex = new RegExp(`^${code.pattern}$`, 'u');
    }
...
function validate(opts) {
    ...
    if (!opts.serviceId) {
        // eslint-disable-next-line no-param-reassign
        opts.serviceId = 1; // Default
    }

    if (opts.immutable === null ||
