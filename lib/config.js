...
function validate(opts) {
    ...
    if (! (opts.policy instanceof Object)) {
        return [false,
                new InvalidConfigError('policy',
                                       opts.policy,
                                       'Expected data placement policy')];
    }

    if (! (opts.policy.locations instanceof Array)) {
        return [false,
                new InvalidConfigError('policy.locations',
                                       opts.policy.locations,
                                       'Expected an array of string')];
    }

    if (opts.policy.locations.some(elem => typeof(elem) !== 'string')) {
        return [false,
                new InvalidConfigError('policy.locations',
                                       opts.policy.locations,
                                       'Expected an array of string')];
    ...
    if (opts.policy.locations.length === 0) {
        return [false,
                new InvalidConfigError('policy.locations',
                                       opts.policy.locations,
    ...
    if (opts.policy.locations.length < opts.dataParts + opts.codingParts) {
        return [false,
                new InvalidConfigError(
                    'totalParts', opts.dataParts + opts.codingParts,
