...
class HyperdriveClient {
    ...
    setupUuidMapping(uuidmapping) {
        let rawUuidmapping = null;
        if (typeof(uuidmapping) === 'string') {
            const loaded = fs.readFileSync(uuidmapping);
            rawUuidmapping = JSON.parse(loaded);
        } else {
            // Consider object as a valid map
            // This is used only for test and internal scripts anyway
            rawUuidmapping = uuidmapping;
        }

        this.uuidmapping = new Map();
        for (const key in rawUuidmapping) {
            if (rawUuidmapping.hasOwnProperty(key)) {
                const url = new urllib.URL(`http://${rawUuidmapping[key]}`);
                this.uuidmapping[key] = { hostname: url.hostname,
                                          port: url.port };
            }
        }
    }
