 ...
 */
function _createHttpRequest(options, logger, callback) {
...
class HyperdriveClient {
    ...
    _newRequest(options, logger, callback, timeoutCallback) {
        ...
        const request = _createHttpRequest(options, logger, callback);
    ...
    put(stream, size, keyContext, reqUids, callback) {
        ...
        const { hostname, port, key } = parts.chunks[0].data[0];
        ...
        const request = this._newRequest(
            ...
            (err, httpResponse) => {
                ...
                let error = null;
                const putTypes = protocol.helpers.parseReturnedContentType(
                    httpResponse.headers['content-type']);
                const storedSize = putTypes ? putTypes.get('data') : -1;
                log.debug('PUT return type: ', putTypes);
                if (storedSize !== size) {
                    error = new Error(
                        `Size mismatch: actual=${size}, stored=${storedSize}`);
                    error.statusCode = 500;
                    log.error(error.message);
                }

                log.end();
                return callback(error, rawGenKey);
            },
            /* timeout callback */
            req => {
                log.end();
