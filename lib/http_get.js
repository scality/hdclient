...
function _erasureGETCb(reqContext, callback, errorAgent) {
    ...
    if (chunkStatus.nOk === k && status.response) {
        ...
        opContext.log.info(`Decode chunk ${reqContext.chunkId}, size=${chunkSize}`);
...
function _fragmentGETCb(reqContext, callback, errorAgent) {
    const fragmentId = reqContext.fragmentId;
    const chunkId = reqContext.chunkId;
    const chunkStatus = reqContext.opContext.status[chunkId];
    const status = chunkStatus.statuses[fragmentId];
    if (status.response) {
        reqContext.opContext.log.info(
            `GET HTTP STREAM ${chunkId}-${fragmentId} - ${status.response.statusCode}`,
            status.response.headers);
    } else {
        reqContext.opContext.log.error(
            `GET HTTP STREAM ${chunkId}-${fragmentId} - ${status.error.message}`);
    }
...
function _chunkGETCb(chunkId, reply, multiplexedStream, args) {
...

    {
        ...
        reply.on(
            'data', chunk => {
                read += chunk.length;
                opContext.log.info(`CHUNK=${chunkId} -> read=${read}`);
            });
    }
    ...
    if (!opContext.firstProcessedChunk) {
        ...
        let read = 0;
        multiplexedStream.on(
            'data', chunk => {
                read += chunk.length;
                opContext.log.info(`read=${read} / ${opContext.fragments.size} cur=chunkId=${chunkId}/${opContext.fragments.nChunks}`);
            });
    }
    ...
    reply.on('error', () => {
        ...
        opContext.log.error('StreamError', error);
        multiplexedStream.emit('error', error);
    });
    ...
    reply.once('end', () => {
        opContext.log.info(`CHUNK=${chunkId} -> END`);
        chunkGET(
...
function chunkGET(multiplexedStream, chunkId, baseFragmentGETArgs) {
    ...
    opContext.log.info(`In chunkGET ${chunkId} / ${opContext.fragments.nChunks}`);
    ...
    if (chunkId === opContext.fragments.nChunks) {
        ...
        opContext.log.end().info('End GET');
        multiplexedStream.end();
        return;
    }
    ...
    if (!use) {
        opContext.log.info(`In chunkGET SKIP ${chunkId} / ${opContext.fragments.nChunks}`);
