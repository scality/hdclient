...
function keygen(endpoints, objectKey, nDataParts, nCodingParts,
    ...
    for (let i = 0; i < nDataParts; ++i) {
        parts.data.push(makeFragment(parts, endpoints[pos], 'd', i));
        pos = (pos + 1) % len;
    }
    for (let i = nDataParts; i < nDataParts + nCodingParts; ++i) {
        parts.data.push(makeFragment(parts, endpoints[pos], 'c', i));
        pos = (pos + 1) % len;
    }
...
function deserialize(rawKey) {
    const [version, topoVersion, split, repPolicy,
           objectKey, rand, ...locations] =
              rawKey.split(SECTION_SEPARATOR);
    if (Number(version) > KEYSCHEME_VERSION) {
        throw new KeySchemeDeserializeError(`Unknown version ${version}`);
    }
    if (Number(topoVersion) > TOPOLOGY_VERSION) {
        throw new KeySchemeDeserializeError(
            `Unknown topology version=${topoVersion}`);
    }
    if (!split) {
    ...
    const nDataParts = Number(nDataPartsStr);
    const nCodingParts = nCodingPartsStr ? Number(nDataPartsStr) : 0;
