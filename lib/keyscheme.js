 ...
 */
function makeFragment(context, location, startOffset, type, fragmentId) {
...
function keygen(policy, objectKey, size, code, nDataParts, nCodingParts,
    ...
    const { nChunks, splitSize } = split.getSplitSize(
        policy.minSplitSize, size);
    ...
    parts.chunks = range(nChunks).map(chunkid => {
        const startOffset = splitSize * chunkid;
        const data = dataLocations.map(
            (e, i) => makeFragment(parts, e, startOffset, 'd', i));
        const coding = codingLocations.map(
            (e, i) => makeFragment(parts, e, startOffset, 'c', nDataParts + i));
        return { data, coding };
    });
...
function serializeSplitSection(parts) {
    return `${parts.nChunks}${SUBSECTION_SEPARATOR}${parts.splitSize}`;
}
...
function deserialize(rawKey) {
    ...
    const [nChunksStr, splitSizeStr] =
              splitSection.split(SUBSECTION_SEPARATOR);
    const nChunks = parseInt(nChunksStr, 10);
    const splitSize = parseInt(splitSizeStr, 10);
    if (isNaN(nChunks) || isNaN(splitSize) ||
        nChunks < 1 || splitSize < 0) {
    ...
    parsed.chunks = range(nChunks).map(chunkid => {
        const startOffset = splitSize * chunkid;
        const data = dataLocations.map(
            (e, i) => makeFragment(parsed, e, startOffset, 'd', i));
        const coding = codingLocations.map(
            (e, i) => makeFragment(parsed, e, startOffset,
                                   'c', nDataParts + i));
        return { data, coding };
    });
