 ...
 * <storedFragmentKey> := <serviceId>-<ctime>-<hash>-<startOffset>-<fragid>
 ...
 * @param {Number} startOffset - Offset inside object (cf split Design.md)
 * @param {Number} fragmentId - Fragment position in the code
 * @returns {String} Generated key
 */
function fragmentKeygen(serviceId, ctime, hash, startOffset, fragmentId) {
    return [serviceId, ctime, hash, startOffset, fragmentId]
 ...
 */
function makeFragment(context, uuid, startOffset, fragmentId) {
    return {
        fragmentId,
        uuid,
        key: fragmentKeygen(context.serviceId, context.ctime,
                            context.hash, startOffset, fragmentId),
    };
}
...
function keygen(serviceId, policy, objectContext, size,
    ...
    parts.chunks = utils.range(nChunks).map(chunkid => {
        ...
        const data = dataLocations.map(
            (e, i) => makeFragment(parts, e, startOffset, i));
        const coding = codingLocations.map(
            (e, i) => makeFragment(parts, e, startOffset, nDataParts + i));
        return { data, coding };
    });
...
function deserialize(rawKey) {
    ...
    parsed.chunks = utils.range(nChunks).map(chunkid => {
        ...
        const data = dataLocations.map(
            (e, i) => makeFragment(
                parsed, e, startOffset, i));
        const coding = codingLocations.map(
            (e, i) => makeFragment(
                parsed, e, startOffset, nDataParts + i));
        return { data, coding };
    });
