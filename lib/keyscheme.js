 ...
 * <rand> := 64bits - in hexadecimal - making the fragment keys unique.
 ...
 * <storedFragmentKey> := <serviceId>-<ctime>-<rand>-<endOffset>-<fragid>
 * <serviceId>, <ctime> and <rand> are the ones defined above.
 * <fragid>:= index in main key fragment list
 * <endOffset> := used for splits, exclusive. All split chunks share the
 *                same prefix, storing the offset is used to easily have
 *                range queries and avoid storing them all in the main key.
 *                End of chunk offset is used to be able to infer object
 *                real size from last chunk keys alone (only way to do it
 *                for erasure coded last chunk parts).
 */
...
/**
 * Sample pseudo-random number in [0, 2**32[
 *
 * @return {Number} pseudo-random number
 */
function rand32() {
    /* Don't use bitshift here. Shift operators
     * work as expected only on 31bits...
 ...
 * Random hash
 *
 * @param {Object} keyContext to hash
 * @return {String} 64bit, hex encoded string
*/
function randhash() {
    /* We don't sample 64bit numbers directly
     * since JS cannot represent natively integers
     * larger than 2**52
     */
 ...
 * @param {String} ctime - Key creation timestamp
 * @param {String} rand - Object random identifier
 * @param {Number} endOffset - Exclusive offset inside object
 *                             (cf split Design.md)
 * @param {Number} fragmentId - Fragment position in the code
 * @returns {String} Generated key
 */
function fragmentKeygen(serviceId, ctime, rand, endOffset, fragmentId) {
    return [serviceId, ctime, rand, endOffset, fragmentId]
        .join(PART_KEY_SEPARATOR);
}
 ...
 * @param {Number} endOffset inside the whole object
 *                           (exclusive, see split Design.md)
 * @param {Number} fragmentId position in the used code
 * @returns {Object} Generated fragment object
 */
function makeFragment(context, uuid, endOffset, fragmentId) {
    return {
        fragmentId,
        uuid,
        key: fragmentKeygen(context.serviceId, context.ctime,
                            context.rand, endOffset, fragmentId),
    };
}
 ...
 *
 * @param {Number} serviceId Service identifier of key creator
 * @param {Object} policy Data placement policy
 * @param {String} objectContext { bucketName, objectKey, version }
 ...
 * @returns {null | Object} parts Part locations, null on error
 ...
 */
function keygen(serviceId, policy, objectContext, size,
                code, nDataParts, nCodingParts) {
    const { nChunks, splitSize, stripeSize } = split.getSplitSize(
        policy.minSplitSize, size, code, nDataParts);

    let selected = null;
    for (let i = 0; i < 10; i++) {
        selected = locationSelector(policy, nDataParts, nCodingParts);
        if ((selected.dataLocations.length > 0 &&
             selected.dataLocations.some(e => e === null)) ||
            (selected.codingLocations.length > 0 &&
             selected.codingLocations.some(e => e === null))) {
            /* We failed to sample appropriately, error */
            selected = null;
        } else {
            break;
        }
    ...
    const { dataLocations, codingLocations } = selected;

    const rand = randhash(objectContext);
    const parts = {
        serviceId,
        rand,
    ...
    parts.chunks = utils.range(nChunks).map(chunkid => {
        ...
        const data = dataLocations.map(
            (e, i) => makeFragment(parts, e, endOffset, i));
        const coding = codingLocations.map(
            (e, i) => makeFragment(parts, e, endOffset, nDataParts + i));
        return { data, coding };
    });
...
function deserialize(rawKey) {
    const [versionStr, serviceIdStr, splitSection, repPolicy,
           ctime, rand, ...locations] =
    ...
    if (isNaN(serviceId) || serviceId < 1) {
        throw new KeySchemeDeserializeError(
            `Unknown serviceId ${serviceIdStr}`);
    }
    if (!splitSection) {
    ...
    if (!ctime) {
        throw new KeySchemeDeserializeError('Bad key: no ctime section');
    }
    ...
    parsed.chunks = utils.range(nChunks).map(chunkid => {
        ...
        const data = dataLocations.map(
            (e, i) => makeFragment(
                parsed, e, endOffset, i));
        const coding = codingLocations.map(
            (e, i) => makeFragment(
                parsed, e, endOffset, nDataParts + i));
        return { data, coding };
    });
