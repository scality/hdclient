 ...
 * <storedFragmentKey> := <serviceId>-<ctime>-<hash>-<endOffset>-<fragid>
...
/**
 * Hash key context object
 *
 * @param {Object} keyContext to hash
 * @return {String} 64bit, hex encoded hash
*/
 ...
 * @param {String} hash - Object hashed identifier
 * @param {Number} endOffset - Exclusive offset inside object
 *                             (cf split Design.md)
 * @param {Number} fragmentId - Fragment position in the code
 * @returns {String} Generated key
 */
function fragmentKeygen(serviceId, ctime, hash, endOffset, fragmentId) {
    return [serviceId, ctime, hash, endOffset, fragmentId]
...
function makeFragment(context, uuid, endOffset, fragmentId) {
    return {
        ...
        key: fragmentKeygen(context.serviceId, context.ctime,
                            context.hash, endOffset, fragmentId),
    };
}
...
function keygen(serviceId, policy, objectContext, size,
    ...
    const hash = keyhash(objectContext);
    const parts = {
        serviceId,
        hash,
...
function deserialize(rawKey) {
    ...
    if (!hash) {
        throw new KeySchemeDeserializeError('Bad key: no hash section');
    }
