...
function chunkedStreamDemux(inputStream, size,
    ...
    let chunkStream = new stream.Duplex();
...
function replicationStreamDemux(
    ...
    const replicatedStreams = [...Array(nReplica).keys()]
              .map(() => new stream.Transform(transformation));
...
function decidePUT(opContext) {
    ...
    if (success) {
        return { error: null, errorHandler: null };
    }

    /* Add every valid fragment to be delete async */
    const toDelete = [];
    let worstError = null;
    opContext.status.forEach((chunk, chunkId) => {
        chunk.statuses.forEach((status, fragmentId) => {
            if (_isReplyOk(status)) {
...
function fragmentPUT(inputStream, size, fragmentId, args) {
    ...
    const request = httpUtils.newRequest(
        ...
        reqCtx => {
            ...
            const { error, errorHandler } = decidePUT(opCtx);
            if (errorHandler &&
                errorHandler.toDelete.length !== 0) {
                _handleErrors(opCtx, errorAgent,
                              errorHandler,
                              callback, error);
                return null;
            }

            return callback(error, opCtx.rawKey);
        });
