...
const { chunkedStreamDemux } = require('./split');
...
function replicationStreamDemux(
    ...
    utils.range(opContext.fragments.nDataParts).forEach(ridx => {
        ...
        inputStream.on('end', () => ostream.end());
    });
}
...
function erasureStreamDemux(
    ...
    /* Properly end the requests */
    [...dataStreams, ...codingStreams].forEach(s => s.on('end', () => s.end()));
...
function decidePUT(opContext) {
    /*
     * Failure to put many fragments should fail
    ...
    const perChunkMinValid = opContext.fragments.code === 'CP' ?
              Math.ceil((opContext.fragments.nDataParts + 1) / 2) :
              opContext.fragments.nDataParts + 1;

    const success = opContext.status.every(
        chunk => (chunk.nOk >= perChunkMinValid));

    /* If PUT is successful, notify every timeout fragment
     * for async check/repair.
...
function _handleErrors(opContext, errorAgent, errorObj,
    ...
    if (errorObj.repair) {
        promises.push(errorAgent.produce(
            ...
            0, // topic partition
            JSON.stringify(errorObj.repair),
            opContext.fragments.objectKey
        ).catch(err => err));
    }
...
/**
 * Handle response to a fragment write
 *
 * @param {Object} reqContext - Request context
 * @param {Object} errorAgent - Agent to persist errors
 * @param {HyperdriveClient~putCallback} callback - Callbackack
 * @return {Object} Whatever callback returned
 */
function fragmentPUTCb(reqContext, errorAgent, callback) {
    const opCtx = reqContext.opContext;
    ...
    if (errorHandlers.delete || errorHandlers.repair) {
...
function fragmentPUT(size, fragmentId, args) {
    const { opContext, chunkId, httpAgent, errorAgent,
            callback, requestTimeoutMs, immutable, force } = args;
    ...
    const requestOptions = httpUtils.getCommonStoreRequestOptions(
        httpAgent, hostname, port, key, queryString);
 ...
 * @param {boolean} immutable - Request hyperdrive to check non-existence
 *                              of key at creation time
 * @param {boolean} force - Request hyperdrive to force the PUT
        ...
        (chunkStream, chunkSize, chunkId) =>
            demux(
                ...
                { opContext, chunkId, httpAgent, errorAgent,
                  callback, requestTimeoutMs, uuidmapping, immutable, force })
        ,
        opContext);
