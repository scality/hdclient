...
function chunkedStreamDemux(inputStream, size,
    ...
    const identityTransforma = {
        // Identity transform
        transform(chunk, encoding, callback) {
            this.push(chunk);
            callback();
        } };
    let chunkStream = new stream.Transform(identityTransform);
    ...
    inputStream.on('data', chunk => {
        ...
        do {
            ...
            if (readSize === nextBoundary) {
                ...
                chunkStream = new stream.Transform(identityTransform);
...
function replicationStreamDemux(
    ...
    const replicatedStreams = [...Array(nReplica).keys()]
              .map(() => new stream.Transform(identityTransform));
